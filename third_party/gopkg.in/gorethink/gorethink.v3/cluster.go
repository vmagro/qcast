...
func (c *Cluster) discover() {
	...
	for {
		backoff.RetryNotify(func() error {
			...
			glog.Warningf("Error discovering hosts %s, waiting: %s", err, wait)
		})
	}
}
...
func (c *Cluster) connectNodes(hosts []Host) error {
	...
	for _, host := range hosts {
		...
		if err != nil {
			attemptErr = err
			glog.Warningf("Error creating connection: %s", err.Error())
			continue
		}
		defer conn.Close()
		...
		if c.opts.DiscoverHosts {
			...
			if err != nil {
				glog.Warningf("Error building query: %s", err)
				continue
			}
			...
			if err != nil {
				attemptErr = err
				glog.Warningf("Error fetching cluster status: %s", err)
				continue
			}
			...
			for _, result := range results {
				...
				if err == nil {
					...
					attemptErr = err
					glog.Warningf("Error connecting to node: %s", err)
				}
			}
		} else {
			...
			if err != nil {
				attemptErr = err
				glog.Warningf("Error fetching server ID: %s", err)
				continue
			}
